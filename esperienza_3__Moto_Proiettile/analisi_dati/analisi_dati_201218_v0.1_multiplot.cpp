...
int main(int argc, char* argv[])
{
	...
	TCanvas parabolic("parabolic", "", 600, 500);	// IMPOSTARE w, h
    parabolic.SetMargin(0.12, 0.06, 0.12, 0.10); 	// Set canvas size and borders
	...
	for(int i=0; i<x_avg_SMALL_v.size(); i++){
		...
		_g_BIG.SetPointError(i, (1e-3)/sqrt(3), ey_v[i]);
	...
	header.DrawLatexNDC(0.12, 0.93, "#splitline{#bf{Paragone tra due masse}}{#bf{in caduta libera}}");//"#bf{Caduta libera PARABOLICA}");
	...
	// flineare.SetParameter(0, 0);
	// flineare.SetParameter(1, n_pre_non_usare);
	...
	string filename="../fig/fig_"+__input_main.substr(slash+1, dot-slash-1)+"multiplot.pdf";
	...
	// auto v0_fit=sqrt(g/(2*n_fit*pow(cos(thet_fit), 2)));
	// auto ev0_fit=sqrt(
	// 	pow(
	// 		((sqrt(g/(double)2)*pow(n_fit, -1.5)*en_fit)/(2*cos(thet_fit))), 2
	// 	)+
	// 	pow(
	// 		((sqrt(g/(double)2)*pow(n_fit, -0.5)*sin(thet_fit)*e_thet_fit)/pow(cos(thet_fit), 2)), 2
	// 	)
	// );
	...
	// risultati<<test_statErr(v0_R, v0_fit,ev0_R, ev0_fit)<<endl;
